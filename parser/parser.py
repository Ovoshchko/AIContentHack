...
class ImageParser:
    ...
    def get_name_to_image_dict(self, name_to_link_dict):
        return {name: self.extract_image(link) for name, link in name_to_link_dict.iteritems()}
    ...
    @staticmethod
    def get_image_links_from_html(html_content, format: str = '.jpg'):
        soup = BeautifulSoup(html_content)

        img_source = [x['src'] for x in soup.find_all("img")]

        return [x for x in img_source if x.startswith('https://') & x.endswith(format)]
